...
const allSupervisors: ActiveSupervisor[] = [
  ...
  { id: 5, name: 'Layla Ibrahim', type: 'Voice Agent', performance: 93, activeCalls: 1, totalToday: 40, failed: 0 },
  { id: 6, name: 'Omar Hassan', type: 'Chat Agent', performance: 86, activeCalls: 3, totalToday: 58, failed: 3 },
];

const myAgents: ActiveSupervisor[] = [
  { id: 1, name: 'Agent-1', type: 'Voice Agent', performance: 82, activeCalls: 2, totalToday: 35, failed: 2 },
  { id: 2, name: 'Agent-2', type: 'Voice Agent', performance: 88, activeCalls: 1, totalToday: 28, failed: 1 },
];

const initialLeaderBoard: LeaderboardSupervisor[] = [
...
const AdminDashboard: React.FC = () => {
  ...
  const [leaderBoard, setLeaderBoard] = useState<LeaderboardSupervisor[]>(initialLeaderBoard);
  ...
  const activeSupervisors = isAdmin ? allSupervisors : myAgents;
  const getNextId = () => Math.max(...leaderBoard.map(s => s.id), 0) + 1;
  ...
  const handleAddSubmit = async (data: { name: string; type: 'voice' | 'chat'; email: string }) => {
    setIsAdding(true);
    const newSupervisor: LeaderboardSupervisor = {
      id: getNextId(),
    ...
    setLeaderBoard(prev => [...prev, newSupervisor]);
    setIsAdding(false);
  ...
  const avgPerf = Math.round(activeSupervisors.reduce((s, a) => s + a.performance, 0) / activeSupervisors.length);
