...
async def update_interaction_status(
    ...
    if current_user.role == "admin":
        ...
        # DEBUG: Print IDs to console to see mismatch
        print(f"DEBUG: Update Interaction {interaction_id}")
        print(f"DEBUG: User {current_user.id} (Role: {current_user.role})")
        print(f"DEBUG: Interaction Agent ID: {existing.agent_id}")
        print(f"DEBUG: Allowed Agents: {allowed_agent_ids}")
