...
- [ ] Start backend: `python run_dev.py` (from backend folder)
...
This will:
1. Create localtunnel for Telegram
2. Update Telegram webhook URL
3. (Optional: Set up Twilio/Meta webhooks if credentials provided)

### For Production
Update `setup_webhook.py` to:
1. Use your production domain
2. Call Telegram API with prod URL
3. Call Twilio API with prod URL (if using)
4. Call Meta API with prod URL (if using)
...
| Localtunnel expires | Session timeout | Restart `python run_dev.py` |
